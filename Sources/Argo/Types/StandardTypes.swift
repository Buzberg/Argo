...
public extension Collection where Iterator.Element: Decodable, Iterator.Element == Iterator.Element.DecodedType {
  ...
  */
  static func decode(_ value: Value) -> Decoded<[Iterator.Element]> {
    switch value {
...
/**
  ...
  This is a convenience function that is the same as `[T].decode(value)` (where `T`
  ...
  - returns: A decoded array of values
*/
public func decodeArray<T: Decodable>(_ value: Value) -> Decoded<[T]> where T.DecodedType == T {
...
public extension ExpressibleByDictionaryLiteral where Value: Decodable, Value == Value.DecodedType {
  ...
  */
  static func decode(_ value: Argo.Value) -> Decoded<[String: Value]> {
    switch value {
...
/**
  ...
  If the `Value` is a dictionary of `String`/`Value` pairs, this returns a
  decoded dictionary of key/value pairs by mapping the value's `decode`
  function over the `Value` and then applying `sequence` to the result. This
  makes `decodeObject` an all-or-nothing operation (See the documentation for
  `sequence` for more info).

  If the `Value` is not a dictionary, this returns a type mismatch.

  This is a convenience function that is the same as `[String: T].decode(value)`
  ...
  - returns: A decoded dictionary of key/value pairs
*/
public func decodeObject<T: Decodable>(_ value: Value) -> Decoded<[String: T]> where T.DecodedType == T {
  return [String: T].decode(value)
}
