...
/**
  ...
  This operator is used to decode a mandatory value from the `Value`. If the
  decoding fails for any reason, this will result in a `.Failure` being
  returned.

  - parameter value: The `Value` object containing the key
  - parameter key: The key for the object to decode

  - returns: A `Decoded` value representing the success or failure of the
             decode operation
*/
public func <| <A: Decodable>(value: Value, key: String) -> Decoded<A> where A == A.DecodedType {
  return value <| [key]
}
...
/**
  ...
  This operator is used to decode an optional value from the `Value`. If the key
  isn't present in the `Value`, this will still return `.Success`. However, if
...
/**
  ...
  - returns: A `Decoded` value representing the success or failure of the
             decode operation
*/
public func <| <A: Decodable>(value: Value, keys: [String]) -> Decoded<A> where A == A.DecodedType {
  return flatReduce(keys, initial: value, combine: decodedValue) >>- A.decode
}
...
/**
  ...
  This operator is used to decode an optional value from the `Value`. If any of
  the keys in the key path aren't present in the `Value`, this will still return
...
*/
public func <|? <A: Decodable>(value: Value, keys: [String]) -> Decoded<A?> where A == A.DecodedType {
  switch flatReduce(keys, initial: value, combine: decodedValue) {
...
/**
  ...
  This operator is used to decode a mandatory array of values from the `Value`.
...
/**
  ...
  This operator is used to decode an optional array of values from the `Value`.
  If the key isn't present in the `Value`, this will still return `.Success`.
...
*/
public func <|| <A: Decodable>(value: Value, keys: [String]) -> Decoded<[A]> where A == A.DecodedType {
  return flatReduce(keys, initial: value, combine: decodedValue) >>- Array<A>.decode
}
...
/**
  ...
  This operator is used to decode an optional array of values from the `Value`.
  If any of the keys in the key path aren't present in the `Value`, this will
